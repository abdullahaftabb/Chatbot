...
def admin():
    ...
    if request.method == 'POST':
        if 'add_update' in request.form:
            ...
            total_availed_sick_leaves = int(request.form.get('total_availed_sick_leaves', 0))
            total_availed_annual_leaves = int(request.form.get('total_availed_annual_leaves', 0))
            total_availed_wfh = int(request.form.get('total_availed_wfh', 0))
        ...
        elif 'update_attendance' in request.form:
            # Updating a specific attendance record
            user_id = request.form['user_id']
            total_availed_sick_leaves = int(request.form.get('total_availed_sick_leaves', 0))
            total_availed_annual_leaves = int(request.form.get('total_availed_annual_leaves', 0))
            total_availed_wfh = int(request.form.get('total_availed_wfh', 0))
